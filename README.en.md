# knowledge-base / obsidian-kb

A **two-skill combo** for general knowledge-base management: one defines *how the
...
In one sentence: **knowledge-base answers "how the knowledge base should look and
flow"; obsidian-kb answers "what constraints apply to tools & operators, and how
to operate Obsidian concretely".**

## Quick start

1. Load `knowledge-base` and follow the onboarding wizard:
...
2. When operating the knowledge base, follow `obsidian-kb` for the Obsidian-side
   execution rules and red lines.

> The tool-layer connection is left to the user to arrange (e.g. loading both
> skills in the agent, or configuring the dispatch relationship); knowledge-base
...
```
├── skills/
│   ├── knowledge-base/     # Skill A: knowledge-base workflow specification
│   │   ├── SKILL.md
...
│   │   └── agents/openai.yaml
│   └── obsidian-kb/        # Skill B: Obsidian operation rules & red lines
│       ├── SKILL.md
│       └── references/     # redlines / cli / markdown / bases / canvas
├── legacy/obsidian-kb/     # archived old obsidian-kb (historical reference only)
├── scripts/update_skill.py # dev-time release tool (packages both skills, not shipped)
└── dist/                   # releases: knowledge-base.zip + obsidian-kb.zip
```
...
- **Config-driven**: vault name, directories and preferences all live in
  `knowledge-base.config.json`; no hardcoded names; old configs migratable
