...
| 无法判定 | 不属于上述任何一类 | **询问用户归属**，由用户决定写入哪一模块（不静默收件箱） | 按用户决定 |

> v1.3.0 起**取消"普通 → 收件箱"兜底**：判断不出类型时，向用户说明内容要点并
> 列出可选模块请其决策，确认后再写入；`inboxDir` 配置键仅保留兼容，不再作为
> 默认路由目标。
...
3. CLI 创建：多行 content 优先 shell 单引号内**真实换行**；frontmatter 列表用
   ...
   **超长内容（>4000 字符）改用两步写入**：先用 CLI 创建占位文件
   ...
   属例外清单第 1 条，回复中说明原因），最后仍执行回读校验与写后三件套。
