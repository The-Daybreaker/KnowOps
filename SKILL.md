...
### 看板（vault 根目录看板文件，v2.2.0 可选组件，v2.3.1 调整）

**形态**：**看板.md（唯一看板文件）+ 按内容命名的模块 .base（数据源）**——
各模块数据库（`日程.base`、`问题.base`、`任务.base`…）只为有内容的模块创建，
看板.md 用 `![[<模块>.base#视图名]]` 嵌入各板块；**不存在"总看板"级 base 文件**。

**动态板块（只加载已有模块）**：
1. 生成时用 CLI 检查各模块是否已有笔记（问题 / 任务 / 日程 / 知识 / 项目 / 剪藏 /
   日记），**只为有内容的模块创建对应命名的 .base**（如 `问题.base`、`日程.base`）；
   看板.md 也只写有内容的板块分区。
2. 板块与过滤要点见 `references/bases.md`「看板板块」；视图与过滤写法见
   `@skill:obsidian-bases`。
3. 更新时（操作后流程第 4 条）：某板块**首次出现内容** → 创建对应 .base 并在
   看板.md 增加板块嵌入。

**创建步骤**（初始化同意或用户要求时）：
1. 检查各模块内容 → 为有内容的模块创建 .base（YAML 只含该板块视图，放 vault
   根目录，位置以用户指令为准）；
2. 创建 `看板.md`：分区标题 + `![[<模块>.base#视图名]]` 嵌入 + 使用说明；
...
4. 校验：.base 用查询校验（空结果也算合法）；看板文件读回核对嵌入引用完整。
