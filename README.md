...
一句话：**knowledge-base 回答"知识库该长什么样、流程怎么走"，obsidian-kb
回答"执行时对工具和操作者的约束、Obsidian 具体怎么操作"。**

## 快速开始

1. 加载 `knowledge-base`，按初始化向导接入知识库：
   - 确认 vault 实际路径与名称；
   - 配置默认写入 vault 内隐藏目录 `.config/`（可改选其他位置）；
   - 按需启用 HTML 镜像导出、创建看板。
2. 操作知识库时，Obsidian 侧的具体执行遵循 `obsidian-kb` 的操作规范与红线。

> 工具层连接方式由使用者自行编排（例如让 agent 同时加载两个 skill，或配置
> 调度关系）；knowledge-base 本身不绑定任何工具。

## 目录结构

```
├── skills/
│   ├── knowledge-base/     # Skill A：知识库管理工作流程规范
│   │   ├── SKILL.md
│   │   ├── references/properties.md   # 属性/目录/生命周期领域设计
│   │   ├── scripts/        # kb_config / kb_env / html_export
│   │   ├── assets/user-manual.md      # 用户手册模板
│   │   └── agents/openai.yaml
│   └── obsidian-kb/        # Skill B：Obsidian 操作规范与红线
│       ├── SKILL.md
│       └── references/     # redlines / cli / markdown / bases / canvas
├── legacy/obsidian-kb/     # 旧版 obsidian-kb 存档（仅历史参考，不含新功能）
├── scripts/update_skill.py # 开发期发布工具（双 skill 打包，不进包）
└── dist/                   # 发布产物：knowledge-base.zip + obsidian-kb.zip
```
...
- **知识库无关文件默认收在 vault 内隐藏目录 `.config/`**（配置/日志/手册/导出），
  不写入用户笔记内容区、不影响笔记浏览；
- **配置驱动**：vault 名、目录、偏好全在 `knowledge-base.config.json`，不假设
