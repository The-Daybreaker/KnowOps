...
- **数据安全红线**：删除永远进系统回收站且可恢复、改删移前征求同意、不代为
...
2. 初始化向导会逐步确认：vault 路径与名称、00–08 九个模块默认目录结构与配置位置
   （默认 vault 内隐藏目录 `.config/`）、插件集成规则（写入 `08 系统管理/Agent规则.md`）、
   08 系统管理模板、06 看板，可选 HTML 镜像导出。
