#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""html_export.py - knowops HTML 镜像导出（Python 标准库，跨平台）
...
def render_page(title: str, body_html: str, frontmatter: dict[str, str],
    ...
    parts.append("<footer>由 knowops html_export 生成</footer>")
    parts.append("</main>")
    if has_mermaid:
...
def render_index(vault_name: str, entries: list[tuple[str, float]]) -> str:
    ...
    if current is not None:
        parts.append("</ul>")
    parts.append(f"<footer>共 {len(items)} 篇 · 由 knowops html_export 生成 · "
