...
2. 展示并确认**基础骨架**：`00 收件箱` / `01 知识` / `03 系统` / `04 归档`
   ...
   仓库外选项）。**懒加载**：初始化预置 03 系统（用户手册、模板）与 01 知识
   （示例主题文档），并创建根目录看板；收件箱、归档首次写入时自动出现。
...
5. **GitHub 暂存库同步配置（可选）**：确认是否启用、仓库（owner/repo）、分支、
   ...
   不启用则不写入。
6. 扫描当前知识库可用的插件/扩展能力。
7. 逐个确认插件集成规则（是否纳入、时机与顺序），按 `assets/agent-rules.md` 模板
...
- **agent 读的个性化约束**：`.config/agent-rules.md`（插件规则、额外红线、额外操作、
  旧库约定）；每次变更操作前读取（见 SKILL.md 红线 7）。**内容规则**（模块归属、
  分类、命名）读用户手册「模块规则」章节。
