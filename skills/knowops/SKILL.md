...
3. 读取该 vault 的 `.config/knowops.config.json`（目录名、路径、HTML 导出、GitHub
   暂存库同步等偏好）。
4. 读取该 vault 的 `.config/agent-rules.md`（若存在：插件规则、额外红线、额外操作、
   旧库约定）与 `03 系统/用户手册.md` 的「模块规则」章节（**模块规则的唯一事实
   来源**：各模块装什么、怎么入库、怎么分类、怎么命名，以手册为准，以库中实际
   存在且已登记的文件夹为准）。
...
7. **变更操作前读取 `.config/agent-rules.md` 与用户手册「模块规则」章节**：
   agent-rules 是 agent 行为的个性化约束（插件规则、额外红线、额外操作、旧库
   约定），不存在视为无额外约束；手册「模块规则」是内容规则的唯一事实来源
   （模块归属、分类与命名），按它执行入库、分类与整理，规则覆盖不了时问用户。
