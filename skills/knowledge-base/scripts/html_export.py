#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""html_export.py - knowledge-base HTML 镜像导出（Python 标准库，跨平台）
...
例外说明：本脚本直接读取 vault 源文件——CLI 无法在无 Obsidian 时提供
等价批量读取，且导出本身不要求 Obsidian 运行，属于直接文件访问例外场景。
"""
...
def render_page(title: str, body_html: str, frontmatter: dict[str, str],
    ...
    parts.append("<footer>由 knowledge-base html_export 生成</footer>")
    parts.append("</main>")
    if has_mermaid:
...
def render_index(vault_name: str, entries: list[tuple[str, float]]) -> str:
    ...
    if current is not None:
        parts.append("</ul>")
    parts.append(f"<footer>共 {len(items)} 篇 · 由 knowledge-base html_export 生成 · "
