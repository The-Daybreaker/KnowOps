...
description: 随身记录与电脑端统一入库的配套 skill。在手机/随身设备上调用本 skill 后直接口述要记录的内容（灵感、随手记、待整理内容等），生成符合知识库格式的 markdown 条目，并设置当晚 22:00 提醒；回到电脑端后，由 knowops-navigator 路由加载本 skill 的桌面部分，将暂存内容解析写入 00 收件箱。不依赖特定 agent 或传输通道。
---
...
- 随身端部分**独立自洽**：手机端只安装本 skill 即可完成“记录 → 规范 md →
  22:00 提醒”闭环，不依赖 knowops-workflow / knowops-navigator。
- 桌面端部分由 **knowops-navigator 路由进入**：依赖方向为 knowops-navigator →
  everywhere-note（桌面部分），本 skill 不反向路由到 knowops-navigator。

## 路由

1. 收到记录/入库请求后，先按当前运行环境选择：
   - **移动/随身设备**：加载 `references/mobile-capture.md`，按其中流程执行；
   - **桌面设备**：仅当 knowops-navigator 路由而来（用户提供暂存内容/文件要求入库）
     时，加载 `references/desktop-ingest.md`；桌面端直接说“记一下……”仍由
     knowops-workflow 处理，本 skill 不介入。
...
| `references/desktop-ingest.md` | 桌面设备上由 knowops-navigator 路由，用户提供暂存内容/文件要求入库 |
