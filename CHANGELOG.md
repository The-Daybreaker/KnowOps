...
### 重写双语 README：以使用叙事重构全篇（用户两轮反馈定稿）

- **改动**：重写 `README.md` / `README.en.md`（双语同步、结构对称）。废除
  「特性平铺 + 编号设计理念清单」的旧结构，改为：亲和开场（装好之后的实际
  体验）+ 内容旅程示意图 + 按「记录 → 整理 → 沉淀 → 可追溯」的使用顺序组织
  的四节叙事——记的时候什么都不用想、整理时规矩由用户定、经验沉淀成主题且
  原话原样保留、每一步都留痕且删错可恢复；设计理念融入叙事而非编号罗列。
  库结构图、安装与官方工具 skill 依赖、初始化向导、自然语言指令对照表、定时
  自动入库、仓库结构说明同步重整；工程取舍（委托官方工具、安全内建、渐进式
  加载）分别融入安装、留痕与结构注释，不再单列清单。
- **背景**：经用户两轮反馈——第一版理念表达生硬，第二版开头「你……又希望……」
  属剖析式语气、不够亲和，且分条理念清单不符合阅读习惯、条目过多；第二版定稿。
  用户同时确立面向用户文档的长期写作标准（语气亲和不剖析用户、按用户认知顺序
  叙事、每段抛具体有用信息、理念融入叙事而非编号列条），已记入长期偏好。
- **纯文档改动**：不涉及 `skills/` 运行时文件，无行为变化，无配置 schema 与
  跨端契约变化；真实场景测试经用户确认沿用 v3.0.6 结论（见 TEST-0004）。
